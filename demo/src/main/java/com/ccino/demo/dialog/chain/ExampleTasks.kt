 ...
 * 示例：需要异步请求数据的广告弹窗
 */
class AdTask : DialogTask(priority = 50) {
...
class NoticeTask : DialogTask(priority = 20) {
    override fun onShow(host: LifecycleOwner, activity: ComponentActivity, chain: ChainCallback) {
        Toast.makeText(activity, "高优先级通知: 正在执行", Toast.LENGTH_SHORT).show()
        
        AlertDialog.Builder(activity)
            .setTitle("重要通知")
            .setMessage("点击取消将停止后续所有弹窗")
