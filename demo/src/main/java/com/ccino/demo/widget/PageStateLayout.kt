...
class PageStateLayout @JvmOverloads constructor(
    ...
    fun inject(activity: Activity) =
        inject((activity.findViewById<View>(R.id.content) as ViewGroup).getChildAt(0))
    ...
    fun showEmpty(
        ...
        if (action == null) actionView?.visibility = GONE
        else {
    ...
    private fun setViewState(view: View) = stateViewMap.forEach {
        it.value.visibility = if (view != it.value) GONE else VISIBLE
    }
